import React from 'react';

export const EspFlag = () => (
    <svg width="100%" height="100%" viewBox="0 0 22 16" fill="none" xmlns="http://www.w3.org/2000/svg">
        <g id="artwork">
            <g clipPath="url(#clip0_1448_332698)">
                <rect width="22" height="16" rx="2" fill="#F93939" />
                <path
                    id="vector"
                    d="M19.9048 0H2.09524C0.93807 0 0 0.955126 0 2.13333V13.8667C0 15.0449 0.93807 16 2.09524 16H19.9048C21.0619 16 22 15.0449 22 13.8667V2.13333C22 0.955126 21.0619 0 19.9048 0Z"
                    fill="#F93939"
                />
                <path
                    id="vector_2"
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M0 4.2666H22V11.7333H0V4.2666Z"
                    fill="#FFDA2C"
                />
                <path
                    id="vector_3"
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M9.42852 6.63677V9.32477C9.42852 10.0714 8.72452 10.6688 7.8571 10.6688H5.76186C4.89652 10.6666 4.19043 10.0661 4.19043 9.32264V6.63464C4.19043 6.02451 4.65976 5.51464 5.3051 5.34931C5.49995 4.79464 6.09919 5.29171 6.80948 5.29171C7.52395 5.29171 8.119 4.79784 8.31386 5.35037C8.9571 5.51997 9.42852 6.03091 9.42852 6.63677Z"
                    fill="#D4AF2C"
                />
                <path
                    id="vector_4"
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M9.42878 7.4668H10.4764V10.6668H9.42878V7.4668ZM3.14307 7.4668H4.19069V10.6668H3.14307V7.4668Z"
                    fill="#CBCBCB"
                />
                <path
                    id="vector_5"
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M9.42878 9.6001H10.4764V10.6668H9.42878V9.6001ZM3.14307 9.6001H4.19069V10.6668H3.14307V9.6001Z"
                    fill="#1A47B8"
                />
                <path
                    id="vector_6"
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M9.42878 6.3999H10.4764V7.46657H9.42878V6.3999ZM3.14307 6.3999H4.19069V7.46657H3.14307V6.3999Z"
                    fill="#D4AF2C"
                />
                <path
                    id="vector_7"
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M5.23828 6.3999H6.2859V7.9999H5.23828V6.3999ZM7.33352 8.53324H8.38114V10.1332H7.33352V8.53324Z"
                    fill="#AF010D"
                />
                <path
                    id="vector_8"
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M7.3335 6.3999H8.38112V7.9999H7.3335V6.3999Z"
                    fill="#AE6A3E"
                />
                <path
                    id="vector_9"
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M5.23828 8.5332H6.2859V10.1332H5.23828V8.5332Z"
                    fill="#FFDA2C"
                />
                <path
                    id="vector_10"
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M6.2859 6.40016L5.23828 5.3335H8.38114L7.33352 6.40016H6.2859Z"
                    fill="#AF010D"
                />
                <path
                    id="vector_11"
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M6.28564 4.2666H7.33326V5.33327H6.28564V4.2666Z"
                    fill="#D4AF2C"
                />
            </g>
        </g>
        <defs>
            <clipPath id="clip0_1448_332698">
                <rect width="22" height="16" rx="2" fill="white" />
            </clipPath>
        </defs>
    </svg>
);
